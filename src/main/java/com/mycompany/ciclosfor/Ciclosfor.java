 ...
 */
package com.mycompany.ciclosfor;
...
public class Ciclosfor {    
    ...
    /**
     * para el ciclo for se debe tener en cuenta lo siguiente:
     * for(inicializacion;condicion;aumneto o decremento){ instrucciones }
     * ejemplo: for (int i = 0; i < 10; i++) { */

    int contador; //se declara la variable contador para ejecuatarla en toda la secuencia

    public static void main(String[] args) {
        Scanner entrada = new Scanner(System.in);
        int contador;
        System.out.println("digita el limite de numeros que deseas imprimir: ");
        contador = entrada.nextInt();
        // ciclo for para numeros pares de 2 en 2
        for (int i = 2; i <= contador; i += 2) { // el Valor inicial se declara i+=2, para aumentar la secuencia de 2 en 2
            System.out.print(i + ",");
        }
        //ciclo para numeros en orden descendente de 2 en 2
        System.out.println("\n________________________");
        for (int i = contador; i >= 0; i -= 2) {// el Valor inicial se declara i-=2, para disminuir la secuencia de 2 en 2
            System.out.print(i + ",");
        }
        //ciclo para numeros en orden ascendente de 1 en 1
        System.out.println("\n________________________");
        for (int i = 0; i <= contador; i++) {// el Valor inicial se declara i++, para aumentar la secuenciar de 1 en 1
            System.out.print(i + ",");
        }
        //ciclo para numeros en orden descendente de 1 en 1
        System.out.println("\n________________________");
        for (int i = contador; i >= 0; i--) {// el Valor inicial se declara i--, para disminuir la secuenciar de 1 en 1
            System.out.print(i + ",");
        }
    }
}
...
public class GUIManual extends JFrame {
    ...
    private JPanel jPanelMenuMatriz;
    private JLabel btnMatriz;
    ...
    public String[][] Matriz = new String[5][2];// se declara matriz de 2 columnas y 5 filas. En la primera columna se guarda el nombre de la página visitada,
    public int cont1 = 6; int cont2=5; int cont3 = 5; int cont4 = 5; int cont5 = 5;
    ...
    private void initComponents() {
        ...
        jPanelMenuMatriz = new JPanel();
        btnMatriz = new JLabel();
        ...
        // Pinta la opción de Menú de las veces que se abre cada menu
        pintarMenuMatriz();
    ...
    private void accionHome() {
        if ( cont1 == 0) { //
                    JOptionPane.showMessageDialog(null, "!!!Uff!!! Superaste el limite de Visitas");
                } else {
                    cont1--;
                }
     ...
     * menú de navegaación
     */
    private void pintarMenuMatriz() {
        btnMatriz.setIcon(new ImageIcon(getClass().getResource("/resources/icons/ver.png")));
        btnMatriz.setText("Historial de Registro");
        btnMatriz.setForeground(new java.awt.Color(255, 255, 255));

        JLabel vacioMenuMatriz = new JLabel();
        jPanelMenuMatriz.setBackground(new java.awt.Color(17, 41, 63));
        jPanelMenuMatriz.setPreferredSize((new java.awt.Dimension(220, 35)));
        jPanelMenuMatriz.setLayout(new BorderLayout(15, 0));
        jPanelMenuMatriz.add(vacioMenuMatriz, BorderLayout.WEST);
        jPanelMenuMatriz.add(btnMatriz, BorderLayout.CENTER);
        jPanelMenu.add(jPanelMenuMatriz);

        btnMatriz.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                System.out.println("Matriz de Vistas");
                accionMenuMatriz();
     ...
     * los resultados
     */
    private void accionMenuMatriz() {
        jLabelTop.setText("Visitas");

        // Si no hay resultados cargados, muestra el botón de carga de resultados
        String[] TextCabc = {"PAGINAS", " VISITAS RESTANTES"};
//se crea la tabla matriz de paginas de visitadas
        Matriz[0][0] = "HOME"; Matriz[1][0] = "SELECCIONES"; Matriz[2][0] = "RESULTADOS"; 
        Matriz[3][0] = "DASH SELECCIONES";Matriz[4][0] = "DASH RESULTADOS";
       // se crea la matriz de registro de ingresos
        Matriz[0][1] = Integer.toString(cont1);Matriz[1][1] = Integer.toString(cont2);        
        Matriz[2][1] = Integer.toString(cont3);Matriz[3][1] = Integer.toString(cont4);Matriz[4][1] = Integer.toString(cont5);
                

        JTable tablevisitas = new JTable(Matriz, TextCabc);
        tablevisitas.setRowHeight(50);//altura de la fila
        JScrollPane scrollPane = new JScrollPane(tablevisitas);

        jPanelMain.removeAll();
        jPanelMain.add(scrollPane);
        //jPanelMain.add(MatrizPanel, BorderLayout.PAGE_START);
        jPanelMain.repaint();
        jPanelMain.revalidate();
    }
    ...
    public void pintarTablaSelecciones() {
        ...
        String[] TextCab = {"ID", "Selección", "Continente", "DT", "Nacionalidad DT"};
        JTable table = new JTable(selecciones, TextCab);
...
/**
 * Tomando el proyecto mundial se quiere realizar un control de la cantidad de veces que un usuario puede ingresar 
 * a cada una de las páginas del menú de navegación. Se espera que el usurio pueda ingresar cómo máximo 5 veces 
 * a cada una de las páginas. Si llega a las 5 visitas de la página el sistema le debe mostar una alerta
 * indicando que ya alcanzó el máximo de visitas permitidas. 

Para lograr esto el estudiante debe:

Crear una matriz de 2 columnas y 5 filas. En la primera columna se debe guardar el nombre de la página visitada, 
en la segunda el contador de visitas que inicialmete aranca en 5 para todas las páginas. 
El número de filas está dado por la 5 páginas que tiene actualmente la aplicación.

El número de visitas decrementa a medida que el usuario va navegando en cada una de las páginas.
 
Crear una opción de menú adicional debajo de Dash de Resultados. La opción se debe llamar auditoría.

Si el contador de la página llega a cero, el sistema debe mostar una alerta al usuario indicando que alcanzó el número máximo de visitas. 

Cuando el usuario ingrese a esta opción el sistema debe mostrar una tabla cómo la de la imagen que muestre las visitas disponibles 
que tiene el usuario en cada página
Subir el archivo comprimido el formato .zip
 */
